...
public func la_zero_matrix(rows: Int = 1, columns: Int = 1) -> la_object_t {
	return la_constant_matrix(value: 0.0, rows: rows, columns: columns)
}
...
public func la_ones_matrix(rows: Int = 1, columns: Int = 1) -> la_object_t {
	return la_constant_matrix(value: 1.0, rows: rows, columns: columns)
}

public func objectFromArray(array: [[Double]]) -> la_object_t {
...
extension la_object_t {
	...
	:returns: The la_object_t instance to use in matrix operations
	*/
	public static func objectFromArray(array: [[Double]]) -> la_object_t {
		let rows = array.count
		let columns = array.first?.count ?? 1
		//let totalElements = Int(rows * columns)
	...
	*/
	public static func eye(dimension: Int = 1) -> la_object_t {
...
public extension la_object_t {
	...
	*/
    private func assertStatusIsSuccess(_ status: la_status_t) {
		switch Int32(status) {
